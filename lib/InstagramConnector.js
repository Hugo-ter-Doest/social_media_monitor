...
var seen = {};
// We need to record the geography subscription id for retrieving recent changes
var geography_subscription_id;
...
app.post('/geography', function(req, res) {
  ...
  objects.forEach(function(object) {
    ...
    ig.geography_media_recent(geography_subscription_id, function(err, medias, pagination, remaining, limit) {
...
function InstagramConnector(q) {
  ...
  if (this.instagram_filter.tag_subscription) {
    ...
    ig.add_tag_subscription(this.instagram_filter.tag_subscription_tag, myhost + '/tag', function(err, result, remaining, limit) {
      if (err) {
        console.log('Instagram subscription tag error: ' + err);
      }
      console.log('Instagram subscription for tags: ' + that.instagram_filter.tag_subscription_tag);
    });
  }
  
  if (this.instagram_filter.geography_subscription) {
    // OPTIONS: { [verify_token] }
    ig.add_geography_subscription(that.instagram_filter.geography_subscription_lon, 
                                  that.instagram_filter.geography_subscription_lat, 
                                  that.instagram_filter.geography_subscription_radius, 
      ...
      console.log('Instagram geography subscription: (%s, %s, %s)', 
                  that.instagram_filter.geography_subscription_lon, 
                  that.instagram_filter.geography_subscription_lat, 
                  that.instagram_filter.geography_subscription_radius);
      console.log("Instagram geography subscription: result" + JSON.stringify(result));
      // Register the geography subscription id. Discovered that it is in object_id.
      geography_subscription_id = result.object_id;
    });
  }
  ...
  if (this.instagram_filter.location_subscription) {
    // OPTIONS: { [verify_token] }
    ig.add_location_subscription(this.instagram_filter.location_id, myhost + '/location', function(err, result, remaining, limit) {
