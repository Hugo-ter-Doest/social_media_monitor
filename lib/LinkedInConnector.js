...
//var APIScope = 'r_basicprofile%20r_fullprofile%20r_emailaddress%20r_network%20r_contactinfo%20rw_nus%20rw_groups%20w_messages';
  ...
  var OauthStep1 = function (req, response) {
    ...
    response.writeHead(302, {
      'Location': 'https://www.linkedin.com/uas/oauth2/authorization?response_type=code&client_id=' + 
        this.API_key + '&scope=' + APIScope + '&state=RNDM_' + RandomState(18) + '&redirect_uri=' + callbackURL
    });
    response.end();
  };
  ...
  var OauthStep2 = function (request, response, code) {
    ...
    var options = {
      ...
      path: "/uas/oauth2/accessToken?grant_type=authorization_code&code=" + code + "&redirect_uri=" + callbackURL 
              + "&client_id=" + APIKey + "&client_secret=" + that.API_secret
    };
    var req = https.request(options, function (res) {
      console.log("statusCode: ", res.statusCode);
      console.log("headers: ", res.headers);
      res.on('data', function (d) {
        this.access_token = JSON.parse(d).access_token;
        // STEP 3 - Get LinkedIn API Data
...
function LinkedInConnector(queue) {
  ...
  this.Initialise(function() {
    // Pull group feeds periodically
    // this.API_Call(APICalls['groupPosts']);
  });
