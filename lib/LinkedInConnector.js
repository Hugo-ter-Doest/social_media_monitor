...
LinkedInConnector.prototype.Initialise = function(callback) {
      ...
      if (cookies['LIAccess_token']) {
        ...
        if (!queryObject.code) {
          ...
          OauthStep2(req, response, queryObject.code, function() {
            logger.debug("access_token = " + that.access_token);
            callback();
          });
        }
      }
  ...
  logger.info('Visit %s in your browser to test the LinkedIn Oauth2 API Authentication', myhost + ":" + port);
  ...
  var OauthStep1 = function (req, response) {
    logger.debug("LinkedIn authentication step 1");
  ...
  var OauthStep2 = function (request, response, code, callback) {
    logger.debug("OauthStep2: LinkedIn authentication step2");
    var options = {
    ...
    var req = https.request(options, function (res) {
      res.on('data', function (d) {
        ...
        if ((API_result) && (API_result.access_token)) {
          ...
          logger.debug("OauthStep2: access_token = " + that.access_token);
    ...
    req.on('error', function (e) {
      logger.error("OauthStep2: There was an error with our Oauth Call in Step 2: " + e);
...
LinkedInConnector.prototype.API_call = function(APICall, callback) {
  ...
  req.on('error', function (e) {
    logger.error("There was an error with our LinkedIn API Call: " + e);
  });
  
  req.end();
};

// Process posts: look for keywords
        ...
        if (post.summary) {
          if (post.summary.indexOf(keyword) !== -1) {
            ...
            if (!seen[post.summary]) {
              that.queue.push({"type": "linkedin", "message": post});
              logger.debug("LinkedInConnector.process_group_posts: added new post: " + post.title);
...
// For each group get the posts and process
LinkedInConnector.prototype.get_group_posts = function() {
  var that = this;
  
  logger.debug("LinkedInConnector.get_group_posts()");
...
function LinkedInConnector(queue) {
  ...
  this.Initialise(function() {
    ...
    logger.debug("LinkedInConnector: polling each 3 seconds for new posts");
  });
}
