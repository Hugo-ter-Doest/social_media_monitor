...
APICalls['groupPosts'] = 'groups/4181220/posts:(title,summary,creator)?order=recency';
    ...
    var req = https.request(options, function (res) {
      res.on('data', function (d) {
        ...
        response.writeHead(200, {
          'Set-Cookie': 'LIAccess_token=' + that.access_token + '; Expires=' + ExpiresIn29days
        });
      });
    });
...
function LinkedInConnector(queue) {
  ...
  this.Initialise(function() {
    ...
    that.API_call(APICalls['groupPosts'], function(result) {
      console.log(result);
    });
  });
}
